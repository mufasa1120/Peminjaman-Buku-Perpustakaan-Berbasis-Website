...
// Dapatkan buku serupa berdasarkan genre yang sama
$genre = $buku['genre'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['pinjam'])) {
    // Pastikan user sudah login (siswa)
    if (isset($_SESSION['id_siswa'])) {
        // Cek stok tersedia
        if ($buku['stok'] <= 0) {
            $error_message = "Maaf, stok buku ini sudah habis";
        } else {
            $id_siswa = $_SESSION['id_siswa'];
            $nama_peminjam = $_SESSION['nama'];
            $tgl_pinjam = date('Y-m-d');
            $bulan_pinjam = strtoupper(date('F', strtotime($tgl_pinjam)));
            $tahun_pinjam = date('Y');
            $judul_buku = $conn->real_escape_string($buku['judul_buku']);
            
            // Ambil data No Telp dan kelas dari tabel siswa
            $query_siswa = "SELECT no_telp, kelas FROM siswa WHERE id_siswa = $id_siswa";
            $result_siswa = $conn->query($query_siswa);
            
            if ($result_siswa->num_rows > 0) {
                $data_siswa = $result_siswa->fetch_assoc();
                $notelpon = $conn->real_escape_string($data_siswa['no_telp']);
                $kelas = $conn->real_escape_string($data_siswa['kelas']);
                
                // Kurangi stok buku
                $stok_baru = $buku['stok'] - 1;
                $update_stok = $conn->query("UPDATE buku SET stok = $stok_baru WHERE id_buku = $id_buku");
                
                // Tambahkan ke rekap peminjaman siswa
                $insert_peminjaman = $conn->query("INSERT INTO rekap_peminjaman_siswa 
                                                (id_buku, judul_buku, nama_peminjam, no_telpon, kelas, tgl_pinjam) 
                                                VALUES 
                                                ($id_buku, '$judul_buku', '$nama_peminjam', '$notelpon', '$kelas', '$tgl_pinjam')");
                ...
                if ($insert_peminjaman) {
                    ...
                    $error_message = "Gagal meminjam buku: " . $conn->error;
        ...
        $error_message = "Anda harus login sebagai siswa untuk meminjam buku";
    }
}
?>
